...
export default function Products() {
  ...
  async function handleAddToWishlist(pid) {
    ...
    setWishlist((prevWishlist) => {
      const updatedWishlist = {
        ...prevWishlist,
        [pid]: !prevWishlist[pid], // تبديل حالة المنتج في الـ wishlist
      };
  ...
  return (
    <>
      {isError ? <span className="loader"></span> : null}
      {isLoading ? (
        <div className="flex justify-center items-center my-10">
          <h1 className="loader1 mb-20"></h1>
        </div>
      ) : (
        <div className="grid sm:grid-cols-3 my-5 shadow-lg rounded-lg  md:grid-cols-3 lg:grid-cols-5">
          {data.data.data.map((product) => (
            <div key={product.id} className=" px-2">
              <div className="product shadow-sm p-3 overflow-hidden my-3 rounded-xl shadow-red-500   group ">
                <Link
